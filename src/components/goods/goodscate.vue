    ...
    <el-dialog title="添加分类" :visible.sync="dialogFormVisibleAdd">
      <el-form :model="form">
        ...
        <el-form-item label="分类" label-width="120px">
          ...
          <!-- 
               options:级联选择器数据 
               selectedOptions:级联选择器选中后/默认选择对应的数组索引  
               defaultProp:每级对应的value label children 
          -->
          <el-cascader
...
export default {
  ...
  methods: {
    ...
    async addCate() {
      ...
      // 三种情况
      // 1. 一级分类 selectedOptions==0 cat_pid=0 cat_level=0
      // 2. 二级分类 selectedOptions==1 cat_pid=this.selectedOptions[0] cat_level=1
      // 2. 二级分类 selectedOptions==2 cat_pid=this.selectedOptions[1] cat_level=2
